...
def gradientDescent(w, x, y):
	print("Descending:")
	while(1):
		wPlus = w + (ALPHA * (       np.dot(   x.transpose(), y - sigmoid(np.dot(x,w)))          ))
...
def main():
	...
	w_standardized = gradientDescent(w, x_train_standardized, y_train)
	# w_transformed = gradientDescent(w, x_train_transformed, y_train)
	# w_binarized = gradientDescent(w, x_train_binarized, y_train)

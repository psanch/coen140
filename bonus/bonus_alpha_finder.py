...
def trans():
	...
	w_transformed = gradientDescent(w, x_train_transformed, y_train, 0.0000001)
...
def bin():
	...
	w_binarized = gradientDescent(w, x_train_binarized, y_train,0.01)
...
def bin_findAlpha():
	...
	alpha = 3.700000299e-07
	while ( alpha < 0.00000038):
		print("Alpha:" + str(alpha) + "{")
		w_binarized = gradientDescent(w, x_train_binarized, y_train,alpha)
		if( w_binarized.any() != 100000000):
			alpha = alpha + 0.0000000000000001
			print("}")
			continue
		if(w_binarized == -1):
			print("Got an increase.. Last alpha:")
			print(alpha)
			return alpha
		else:
			
			print("FOUND GOOD ALPHA")
			print(alpha)
			break

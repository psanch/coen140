...
def transform(x_train, x_test):
	...
	for i in range(NUM_TRAINING_INSTANCES):
		for j in range(NUM_FEATURES-1):
			x_train[i][j] = math.log10(x_train[i][j] + 0.1)
	
	for i in range(NUM_TESTING_INSTANCES):
		for j in range(NUM_FEATURES-1):
			x_test[i][j] = math.log10(x_test[i][j] + 0.1)
...
def lossFunction(w, x, y):
	...
	print(rng)
	for i in range(rng):
		r = np.dot(w.transpose(),x[i])
		for j in range(len(r)):
			if(r[j] > 644):
				r[j] = 644.1337
			if(r[j] < -644):
				r[j] = -644.1337
		print("\t\t\twTx: " + str(r) + str(i))
		loss = loss + (y[i]) * math.log( sigmoid(r)) + (1-y[i]) * math.log( sigmoid(r) )
	return loss
...
def trans():
	...
	w_transformed = gradientDescent_bin(w, x_train_transformed, y_train, 0.00001)
